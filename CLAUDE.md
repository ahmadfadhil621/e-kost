...
- Never modify `prisma/schema.prisma` directly. If a feature requires schema changes, describe the needed changes and let the user apply them manually in Supabase. After they confirm the changes are applied, run `npx prisma db pull` then `npx prisma generate` to sync the client. After `prisma generate`, always restart the dev server before running E2E tests — the running server does not hot-reload the regenerated Prisma client, and new model routes will silently return 500 until restarted: `pkill -f "next-server" || true && npm run dev > /tmp/dev-server.log 2>&1 & sleep 8`
