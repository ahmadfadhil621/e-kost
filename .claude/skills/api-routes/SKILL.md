...
1. Read the feature's spec files:
   ...
   - `.claude/rules/coding-standards.md` -- error handling, naming, validation conventions
...
- **Service** (`src/lib/`): Orchestrates business logic, calls repository interface
...
Import shared schemas from `src/domain/schemas/`. Never define schemas inline in route files.
...
Define interfaces in `src/domain/interfaces/`. Implementations go in `src/lib/repositories/prisma/`.

```typescript
// src/domain/interfaces/room-repository.ts
import type { Room, CreateRoomInput, UpdateRoomInput, RoomFilters } from '@/domain/types';
...
3. `.claude/rules/coding-standards.md` -- naming, error handling, TypeScript conventions
