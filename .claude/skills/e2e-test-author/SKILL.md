...
This skill covers **step 2** of the feature development workflow: defining small, atomic E2E tests with Playwright. For the full workflow, see `.claude/rules/testing.md`.
...
1. Read the feature's spec files:
   ...
   - `.claude/rules/testing.md` -- project testing standards (includes E2E strategy)
2. Identify the key user actions for the feature (1 spec file per action)
3. Write Playwright spec files in `e2e/<feature>/`
4. Add traceability comments to each spec file mapping acceptance criteria to tests (same format as unit tests -- see `test-author` skill)
...
6. **Hand off to step 3**: Test quality validation using the `test-validator` skill. All three quality gates must pass before implementation begins.
...
3. `.claude/rules/testing.md` -- project-wide testing conventions and E2E strategy (6-step workflow)
