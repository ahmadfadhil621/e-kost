---
name: test-author
description: Write TDD unit and integration test suites for E-Kost features (workflow step 1). Use when writing Vitest tests for domain services, API routes, or UI components. Covers Good/Bad/Edge structure, property-based tests with fast-check, test fixtures, and MSW handlers. For Playwright E2E tests (workflow step 2), use the e2e-test-author skill instead.
user-invocable: true
allowed-tools:
  - Read
  - Write
...
This skill covers **step 1** of the feature development workflow: defining unit and integration tests with Vitest. For the full workflow, see `.claude/rules/testing.md`.
...
1. Read the feature's spec files:
   ...
   - `.claude/rules/testing.md` -- project testing standards (includes the 6-step workflow)
...
7. **Hand off to step 2**: E2E tests are written next using the `e2e-test-author` skill
...
3. `.claude/rules/testing.md` -- project-wide testing conventions (6-step workflow)
4. `.claude/rules/test-quality-gates.md` -- quality gate checklist (tests will be validated against this)
