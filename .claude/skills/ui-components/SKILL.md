...
1. Read the feature's spec files:
   ...
   - `.claude/rules/coding-standards.md` -- React conventions, naming, Tailwind rules
...
Every form uses the same pattern. Zod schemas come from `src/domain/schemas/`.

```tsx
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { createRoomSchema, type CreateRoomInput } from '@/domain/schemas/room';
...
Use `Intl.NumberFormat` with the configured currency, respecting locale:
...
3. `.claude/rules/coding-standards.md` -- React conventions, naming, Tailwind rules
